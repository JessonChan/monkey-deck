...
- **当前焦点**:右侧新增「文件」管理面板(tab:文件/源代码管理,仿 VSCode 侧栏);继续对话体验打磨
- **最后更新**:2026-06-29(右侧文件管理面板)
- **可运行状态**:✅ 端到端可跑 —— Wails3 单进程 + opencode ACP 多 session 对话、历史恢复(LoadSession)、权限 UI、SQLite 本地落盘、token 用量统计、会话标题(opencode 经 session/list 权威标题 + 瞬时 fallback)、源码管理 SCM、三栏可拖拽分隔线、**右侧文件浏览/管理(树+预览+增删改)**。`go test ./internal/...` 通过、前端 `tsc` + `vite build` 通过。
- **近期改动汇总**:
