...
> ⚠️ **本文件已停维(2026-06-30 起),仅作历史归档保留(只读)。**
> 开发追踪改走 `docs/worklog/`(一条一文件,见其 `README.md`)。**不要再往本文件写新内容**;
> 开工前对齐进度也改读 `docs/worklog/` 最近几条。详见 AGENTS.md §0.3。
>
> ---- 以下为停维前的原始说明,原样保留以示历史 ----
>
> 本文件曾是 monkey-deck 的**活文档 / 开发追踪单**(与 AGENTS.md 分工:AGENTS.md = 规矩,PROCESS.md = 进度)。
> 原要求「任何 agent 动手前必读本文件、收工前必更新本文件」——**该要求已随停维废止**。
...
3. **执行 —— 边做边记**
   ...
   - 踩到坑 → 先记进 AGENTS.md §5.4 + 新建 `docs/worklog/` 条目,再修。

4. **收工前 —— 更新本文件(必做)**
   - §B 进度快照刷新(现在能跑吗 / 卡在哪 / 下一步)。
   - §C 看板:完成的任务 → `done`,未完成的留 `in-progress` 并注明卡在哪。
   - 工作日志:在 `docs/worklog/` 新建一个文件 `YYYY-MM-DD-<slug>.md`(一条一文件,避免多分支合并时 PROCESS.md §G 冲突;约定见 `docs/worklog/README.md`)。
...
- **当前焦点**:布局可调(三栏可拖拽分隔线)+ 源码管理 SCM 化(含审查 5 项修复)+ 会话标题修复 + 右侧文件管理面板(tab:文件/源代码管理)+ **AI 提交/AI 合并(SCM 结合 AI)** 均已完成;继续对话体验打磨(**输入框上下键翻历史 + @ 提及文件/文件夹(经 ACP ResourceLink 发送)**);**model/思考等级/会话模式选择(Composer 下拉热切,ACP Session Config Options)**;**新建会话选择 harness(omp/opencode,默认 omp)+ 是否新建 worktree(默认不建)**
- **最后更新**:2026-06-30(harness 收窄:移除 mimo/mino,仅留 opencode + omp 且 omp 为默认;worktree 默认不建)
- **可运行状态**:✅ 端到端可跑 —— Wails3 单进程 + 多 harness(omp/opencode,默认 omp)ACP 多 session 对话、历史恢复(LoadSession)、权限 UI、SQLite 本地落盘、token 用量统计、会话标题(opencode 经 session/list 权威标题 + 瞬时 fallback)、源代码管理 SCM(提交/暂存/丢弃/单文件 diff/并发守卫/**AI 提交/AI 合并**)、三栏可拖拽分隔线、**右侧文件浏览/管理(树+预览+增删改)**、**窗口尺寸/位置/最大化记忆(ui_state.json)**、**应用自更新(菜单「检查更新…」+ 发布版后台静默检查 + `release:darwin` 打包工具链)**、**model/effort/mode 下拉热切**、**新建会话选 harness + worktree 开关(默认不建)**。`go test ./internal/...` 通过、前端 `tsc` + `vite build` 通过。
- **近期改动汇总**:
  - **harness 收窄 + worktree 默认不建**(2026-06-30):移除 mimo/mino,仅留 opencode + omp;**omp 设为默认**(`harness.DefaultID="omp"`,`Supported` 首项 = omp);**worktree 默认不建**(NewSessionModal `useWorktree` 初值 false)。`IsOpenCode` 改显式判 `"opencode"`(不再随 DefaultID,否则改默认会误把 omp 当 opencode 写 opencode.json)。migration 0005 列默认 'opencode' 为历史记录不动(应用层 CreateSession 始终显式传 harness,列默认是死路径)。详见 `docs/worklog/2026-06-30-trim-harness-default-omp.md`。
  - **合并 md/d4d163ac(harness + worktree 选择)**(2026-06-30):新建会话 harness 选择功能合入 main。冲突仅 App.tsx 状态声明(union:main 的 attachments/mentions/configOptions per-session 态 + 分支 harnesses/newSession)+ PROCESS §B/§G。新增 `internal/harness` 注册表(opencode/mino/omp)+ migration 0005(sessions 加 harness 列)+ store.Session.Harness + `CreateSession(harness,useWorktree)` + `ListHarnesses`/`IsGitProject` 绑定 + `NewSessionModal`;`startLive` 按 session.Harness 解析 spawn。详见 §G。
  - **合并 md/96d8364a(model/effort/mode 选择)**(2026-06-30):该 session 卡死(§5.4 #16)前完成的 model/effort/mode 选择合入 main。冲突 10 处前端取并集 + Composer 弃用过时 setAttachments/静态 model span(ModelSelect 取代,noUnusedLocals);绑定 gitignored,合入新 Go 方法后 `wails3 generate bindings` 重生成。详见 §G。
  - **合并 md/13e08a19(ocr review 输入框修复)**(2026-06-30):该 session 卡死(§5.4 #16)前已完成的 2 commit(composer 6 项修复 + PROCESS §G 日志)合入 main。冲突 2 处取双方融合:**保留 main 受控组件模式**(`onMentionsChange`/`onChange`,跨 session 隔离) **+ 并入分支** pickMention 的 `cursorRef` 同步(防 @ 面板重开)、mention chip X 清 `@path` 文本(防删 chip 后 @path 当纯文本发出、丢 ResourceLink)。PROCESS.md 保留 main 全部条目 + 分支「ocr review 6 项修复」插 06-30 块末尾。6 项修复(navigateHistory `+dir` 方向 / chip 清 @path / submit indexOf+词边界 / cursorPos 作 useMemo 触发器 / SessionListDir 150ms 防抖 / App historySeededRef stale-closure 守卫)均已核实。`tsc` + `vite build`(331 modules)✅。详见 §G。
...
## G. 工作日志(历史归档;新条目进 docs/worklog/)

> ⚠ **2026-06-30 起,新工作日志不再写到这里** —— 改为 `docs/worklog/` 下「一条一文件」(避免多 session 分支同时往本节顶部追加导致的合并冲突,详见 `docs/worklog/README.md`)。本节以下为历史归档,原样保留。
...
- **设计**:
  - 新包 `internal/ui`(`state.go`):`WindowState{x,y,width,height,maximized}` + `LoadWindow`(缺失返回零值 nil)+ `SaveWindow`(原子写:先 `.tmp` 再 rename)。最小尺寸护栏 `MinWidth/MinHeight`(600/400)。
  - `main.go` 接线:启动 `LoadWindow` → 有记录且非最大化时 `InitialPosition=WindowXY` + X/Y 恢复位置;上次最大化则 `StartState=WindowStateMaximised`(最大化时让系统居中逻辑接管,避免 maximise 后再 setPosition 干扰);首次启动无记录由系统居中 + 默认 1280×840。
  - 运行时跟踪:监听 `WindowDidResize`/`WindowDidMove`(仅非最大化时更新正常几何——关键:否则最大化尺寸会被当默认尺寸存下来)、`WindowMaximise`/`WindowUnMaximise`(更新标记)、`WindowClosing`(`SaveWindow` 落盘)。
- **改了哪些文件**:`internal/ui/{state.go(新), state_test.go(新)}`、`main.go`、`PROCESS.md`(本节+§B/§E)。
- **验证**:`go test ./internal/ui/` ✅(4 用例:缺失零值/round-trip+无 .tmp 残留/损坏报错/字段全导出可序列化);`go test ./internal/...` ✅(8 packages);`go vet ./internal/...` 干净;`gofmt -l` 干净;`go build ./...` ✅(临时占位 dist 验证 main.go 编译,ld warning 为 macOS SDK 版本警告无关)。
- **未做实机验证**(待 `wails3 dev`):移动/缩放后关闭再开恢复、最大化后关闭再开最大化、首次启动居中。Cmd+Q 是否触发 WindowClosing 待实机确认(macOS `applicationWillTerminate` 应逐窗口关窗)。
- **下一步**:实机验证;若 Cmd+Q 不触发 WindowClosing,改为「resize/move 后即写(写穿透)」兜底。
