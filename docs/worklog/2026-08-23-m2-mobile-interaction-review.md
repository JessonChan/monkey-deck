...
## 追加(同日第三轮):真机反馈——移动端做减法(commit adcf622)

用户真机测试反馈四点 + 审美批评:「不带审美、太难看,手机聊天不需要桌面那么多信息,直接隐藏,不要机械」。定位修正:从"桌面控件塞进 390px"转向"手机聊天应用形态"。

### 改动

1. **composer 减法**:只留 照片按钮(agent 声明图片能力时)+ 输入框 + 停止/发送。隐藏:斜杠按钮(输入 / 仍唤出菜单)、历史 chip、模型/模式/effort 三个 chips、用量表、分支、MCP、引用计数、入队按钮(prompting 时发送本就入队)。grid 把输入与操作列并成**一行**(原 DOM 是"输入行+按钮行"两层,手机上空态 ~140px 三层堆叠→现 86px),操作列垂直居中。
2. **header 减法**:只留 抽屉开关(汉堡位,top:13,header 让位 46px)+ session 标题 + 状态徽标。隐藏项目名/终端开关/模型/分支/合并按钮。
3. **只读横幅**:全宽大声条 → 27px 细条。
4. **tooltip 触屏彻底关闭**(用户:易误导):`hidden={coarsePointer}`,撤销上一轮 openOnClick 方案;桌面 hover 不变。
5. **session 多标签条隐藏**(用户明确不要;切换走抽屉)。

### 验证

- 390×844:send (316-350) 可见、输入/操作列同行(sameRow=true)、对齐差 5px、横幅 27px、tabbar display:none、header 无项目名。vision 终评:"reads clearly as a mobile chat composer"。
- 桌面像素 diff(第四轮,精确标题对齐同 session):与前三轮完全一致——仅 848px delta-1 头像渐变栅格化差(确定性、不可感知),文字/布局/边缘零差。
- `tsc` 过;`bun test` 全套出现 HarnessPane(7)/SidePanel(1) 失败——**既有调度抖动**(隔离运行 0 fail;2026-08-23-editor worklog 已记录该现象),非本次改动。

### 设计原则沉淀(后续移动端改动遵循)

手机 UI 不是桌面 UI 的压缩版:桌面信息密度(配置 chips/用量/终端/多标签)在手机上是噪音。判断标准:这条信息在"边走边聊"场景下是否当下必需?不是→藏;功能仍可达(输 / 唤菜单、抽屉切会话)即可。

## 追加(同日第四轮):徽标高度钉死(用户第一优先级)

用户:「所有标签不要自适应变高,保持一行;空闲/工作中这类,不变高就行」。此前 dots/idle 徽标 padding 被我调大(6px),造成视觉变高。修:全部状态徽标(文字/点/单点)统一 **height: 22px + 基础水平 padding**,只有字形在换、盒子永不变。E2E 实测六种徽标高度全部 = 22px。
