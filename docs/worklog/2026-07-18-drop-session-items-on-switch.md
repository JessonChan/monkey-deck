...
- **端到端 A/B 验证(2026-07-18,用 Wails3 server 模式浏览器基建,见 §关联)**:
  在 `feat/drop-session-items-on-switch` 分支用 server 模式(`go build -tags server`)+
  Chromium 驱动真 UI × 真 SQLite 数据,做 A→B→A 切换,**抓 `/wails/runtime` POST 的 body** 检测
  `LoadMessagesPage`(methodID `876520863`)是否被调用(确定性信号,不受 V8 GC 噪声影响):
  | build | back-A 调用数 | LoadMessagesPage 被调? | 含义 |
  |---|---|---|---|
  | **WITH 切走丢弃** | 6 | ✅ 是 | A 被丢 → 切回从 DB 重载 |
  | **WITHOUT**(谓词强制 false) | 5 | ❌ 否 | A 缓存命中 → 不重载 |
  结论:切走丢弃**行为正确生效**——切走空闲 session 时清缓存 + loadedSessionsRef 守卫,
  切回触发 DB 重载。单开一个 session 的 `performance.memory` 量到 heap +4.5 MB
  (7.9→12.4 MB,渲染 101 节点),证明 session 数据确有可观内存成本,回收有实际意义。
  ⚠️ V8/Chromium 的绝对 heap 数字因 GC 非确定而噪声大,平台期趋势不明显;
  **绝对内存收益的量化仍需在桌面 WebKit app 上 `vmmap`/`top` 量**(见 §下一步)。
- **内存绝对值与 UX(切回重载 flash)的最终量化由用户在桌面 app 上手动跑**。
