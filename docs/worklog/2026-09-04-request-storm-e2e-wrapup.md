...
2. wails3 CLI 双线并存：PATH 上 beta.16 与 go.mod alpha2.106 不同线；本系列用 `/tmp/md-wails-bin/wails3`（alpha2.106）生成 bindings。建议择期统一（升级 go.mod 到 beta 线或钉住 CLI），AGENTS §0.5「三者同步」目前靠人工。
