...
- **git submodule**:❌ 与 `references/` 被 `.gitignore` 冲突(submodule 必须是被跟踪路径),且 `real-agent-kanban` / `opencode` 本地副本无 remote,挂不上。
- **纯 README 文档**:❌ 不可执行、易漏。
...
## 改法
- 新增 `scripts/references.sh`:`REFERENCES` 表(名称|URL|路径|协议|用途)为单一事实来源;`--list` 打印清单、`--status` 预览、默认浅克隆缺失项、`--pull` 更新、`--full` 完整历史。`URL=-` 表示本地私有(`real-agent-kanban`),脚本只提示不克隆。克隆用 HTTPS(免 SSH key);URL 经本地 `git remote` 核对(`opencode` 本地副本无 remote,按 `sst/opencode` 填并标注请核对)。
...
## 验证
- `bash scripts/references.sh --list`:10 条清单正确列出(URL/协议/用途)。
- `bash scripts/references.sh --status`:10 个全部 ✓ 已存在,汇总 `已存在 10 / 可克隆缺失 0 / 本地私有缺失 0 / 共 10`。
- `bash scripts/references.sh`(无参,全已存在):正确 no-op,汇总 `完成:已存在 10,本次克隆 0`。
- `--help`:从标题行正确打印用法块。
- 修复了一处 `set -u` 未定义变量(`--status` 汇总行原引用了不存在的 `$missing`,改为 `clonable_missing` 计数器)。

## 下一步
- `real-agent-kanban` 若有公开 remote,补进 `REFERENCES` 的 URL 字段(目前 `-` 本地私有)。
- `opencode` 的 `sst/opencode` URL 待人工最终确认。
