...
export default function FilePanel({ sessionId, rootName, changes, status }: Props) {
  ...
  const openFile = async (node: FileNode) => {
    ...
    try {
      ...
      setPreview({ name: node.name, path: node.path, content: "读取失败:" + String(e) });
    }
  };
  ...
  const renderNode = (node: FileNode, depth: number) => {
        ...
        <div key={node.path}>
          ...
          >
            ...
            <span className="tree-acts">
              <button className="tree-act" title="新建文件" onClick={(e) => { e.stopPropagation(); openModal({ kind: "file", dir: node.path }); }}><FilePlus2 size={13} /></button>
              <button className="tree-act" title="重命名" onClick={(e) => { e.stopPropagation(); openModal({ kind: "rename", dir: depth === 0 ? "" : node.path.substring(0, node.path.lastIndexOf("/")), target: node.path, initial: node.name }); }}><Pencil size={12} /></button>
              <button className="tree-act danger" title="删除" onClick={(e) => { e.stopPropagation(); openModal({ kind: "delete", dir: depth === 0 ? "" : node.path.substring(0, node.path.lastIndexOf("/")), target: node.path, isDir: true }); }}><Trash2 size={12} /></button>
            </span>
          </div>
    ...
    return (
      ...
      >
        ...
        <span className="tree-acts">
          <button className="tree-act" title="重命名" onClick={(e) => { e.stopPropagation(); openModal({ kind: "rename", dir: node.path.substring(0, node.path.lastIndexOf("/")), target: node.path, initial: node.name }); }}><Pencil size={12} /></button>
          <button className="tree-act danger" title="删除" onClick={(e) => { e.stopPropagation(); openModal({ kind: "delete", dir: node.path.substring(0, node.path.lastIndexOf("/")), target: node.path, isDir: false }); }}><Trash2 size={12} /></button>
        </span>
      </div>
    );
  };
    ...
    <div className="file-panel" data-testid="file-panel">
      <div className="file-toolbar">
        ...
        <span className="file-toolbar-acts">
          <button className="tool-btn" title="新建文件" onClick={() => openModal({ kind: "file", dir: "" })}><FilePlus2 size={15} /></button>
          <button className="tool-btn" title="新建文件夹" onClick={() => openModal({ kind: "folder", dir: "" })}><FolderPlus size={15} /></button>
          <button className="tool-btn" title="刷新" onClick={refresh}><RefreshCw size={14} /></button>
        </span>
      </div>
      ...
      <div className="tree-body">
        {rootLoading && <div className="tree-empty">加载中…</div>}
        {!rootLoading && rootChildren.length === 0 && <div className="tree-empty">空目录</div>}
