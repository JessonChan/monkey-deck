...
// user_harnesses.go:用户自添加 ACP harness 的持久化(声明即用流程)。
//
// 与内置 harness(harness.Supported/Registry,Go 变量)并列。用户经 ProbeHarness 自检通过后
// 落库一行;启动时 service 层把它和内置项合并成完整 harness 列表(harness.SetUserHarnesses)。
//
// 这里只管 CRUD(§2.1:store 是 SQL 唯一入口);"是否能加"的判定在 ProbeHarness(acp 层),
// "与内置 id 冲突"的校验在 service 层(harness.ValidateUserHarness)。store 不假设这些语义。
...
// UserHarness 用户自添加 harness 一行(只存静态元数据)。
// 运行时数据(安装路径/版本/升级)由 harness.Discover 在合并视图上统一填,不在此存。
...
// CreateUserHarness 新建一个用户 harness。id 由调用方给(避免与内置冲突,已在 service 层校验)。
// icon 空串合法(走兜底)。返回带 created_at 的完整行。id 已存在 → SQLite UNIQUE 约束报错。
...
// GetUserHarness 取单个用户 harness;不存在返回 nil, nil(调用方按内置回退)。
