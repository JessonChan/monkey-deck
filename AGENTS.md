...
**Wails 版本纪律(硬约束)**:Wails3 跟进**最新 alpha**(**wails3 CLI、go module、生成的 bindings 三者版本必须同步**,升级时一起升,**禁止锁旧版**)。改 Go 导出方法签名后必须 `wails3 gen bindings`(dev 起的进程前端 binding 走运行时注入,不重新生成则前端用旧签名)。**bindings 生成必须走 `wails3 task bindings`**(2026-09-04 起该任务动态读 go.mod 的 wails 版本、用匹配版本的 CLI 生成——PATH 上的 wails3 可能处于不同 release 线(beta vs alpha2),其生成器输出格式/类型不兼容,直接跑 `wails3 generate bindings` 已实测产生跨线类型冲突);升级 wails 版本时只改 go.mod,bindings 任务自动跟随。
...
- **功能实现默认选后端（Go）**:某项功能可在后端也可在前端完成、且实现难度差异不大时,**优先在后端**。前端聚焦 UI 渲染与交互,后端承担数据处理、业务逻辑编排与系统能力。
...
| **M2.5** | Capacitor 薄壳 APP(iOS/Android):server URL 模式加载远程服务 + 扫码配对(/pair 一次性配对码)+ token 入钥匙串 + mDNS 发现 + iOS ATS(NSAllowsLocalNetworking)。**复用同一份 React 前端,零 UI 重写**;触发条件:M2 完成且真机验证有安装 APP 的需求 | — |
...
- **外部事实是设计前提时,先验证再动手(硬约束)**:依赖某外部事实(协议字段是否填充、上游行为)的设计,动手前**先用最小成本验证**(读参考实现/打日志/跑探针)——不是为"确认你对",是为"前提错了早 10 分钟知道";同时想清楚主干失败的降级路径。**取样须覆盖输入形态矩阵**:不止 happy path 与垃圾输入两端,还要覆盖「合法但形态不利」的中间态——#24311 P1(trailing-moov m4a 经不可 seek 的 pipe 输入 → ffmpeg exit 0 + 零音频帧 WAV,静默穿透全部错误哨兵)即只在两端取样而漏检的中间态;webm/fMP4 恰好是流式容器掩盖了 pipe 的不可 seek。
...
8. **GUI 双击启动拿不到用户 PATH(已修)**:Finder/Dock 双击启动经 launchd,进程只继承系统默认 PATH(`/etc/paths` + `/etc/paths.d`),**不读** `.zshrc`/`.zprofile` → `~/.bun/bin`(omp)、`~/.opencode/bin`(opencode)等用户级安装位找不到 → harness `Installed=false` + spawn `ErrNotFound`。终端启动继承完整 shell PATH 则正常。**修法**:`internal/shellenv` 包 spawn 用户登录+交互 shell 读其 `env` 输出的 PATH,**union 合并**进进程(shell 目录在前 + 进程已有但 shell 没有的目录在后,绝不丢目录——覆盖会让终端临时加的 `/tmp/extra` 里的 harness 消失);`chat.refreshHarnessesAsync`(Discover 前)与 `acp.spawnAndInit`(`cmd.Start()` 前)两处调用,谁先触发谁干活、另一个秒回缓存。**预算脱离 caller ctx**(`doResolve` 从 `context.Background()` 派生,不接 caller——结果经 `sync.Once` 进程级缓存,若用 `WithTimeout(callerCtx)` 取 min 或让 caller 取消传播进来,短命 caller 超时会毒化缓存让用户再也起不了 session)。**自己不读/不解析** `.zshrc`(只有 shell 自己能正确解释 source 链/条件/函数)。**不做持久化缓存、不碰用户文档**(用户拍板)。spawn 失败 `errors.Is(err, exec.ErrNotFound)` 包人话提示(§4.4)。详见 `docs/worklog/2026-08-01-shell-env-resolve-finder-launch.md`。
9. **`sync.Once` + 局部 channel = 后续调用方永久阻塞(并发反模式)**:`shellenv.Resolve` 初版用 `once.Do(func(){ c <- work() })` + 局部 `c := make(chan error,1)`,然后 `err := <-c`。`once.Do` 只让**第一个** goroutine 进闭包写 `c`;其它 goroutine 不进闭包却都执行 `<-c` 读一个**永远没人写的 channel**(只有进闭包的那个 goroutine 持有 `c` 写入侧)→ 永久阻塞。**正确做法(§5.3 找不变量,不堆 if)**:所有调用方读**同一份共享结果字段**(`resErr`/`didRun`,由 `resMu` 保护),无 per-goroutine channel;`once.Do` 写共享字段,出 `once.Do` 后统一从共享字段读(`once.Do` 是 full barrier,保证可见性)。教训:并发原语组合要验证「谁是生产者、谁是消费者、channel 谁能拿到引用」——`once` + 局部 channel 是经典反模式(消费者集合 ≠ 生产者集合)。
10. **部分 harness 流式 chunk 不带 `messageId`(已加 fallback)**:ACP 把 `messageId` 标为 UNSTABLE/optional,但它是 chunk 归并的契约字段(SDK:"All chunks belonging to the same message share the same messageId; a change indicates a new message has started")。omp/opencode 都发;**Reasonix 不发**——每个 `agent_message_chunk`/`agent_thought_chunk` 是裸 `{sessionUpdate, content}`,逐 token、零边界信号(wire 实测)。**症状**:无 fallback 时 `messageKey` 走 `nextSyntheticID` → 每个 chunk 一个新 entry → 思考/回复碎成一堆单 token 气泡。**修法**:`messageKey`(`internal/chat`)在 `messageId==""` 时走 fallback——连续同 role 的 chunk 累积进同一条 entry,role 变化 / `tool_call` 打断时轮换新 entry(`fallbackRole`/`fallbackSeq`,turn 开始 `resetBuffers` 清)。**这是 §5.3「把启发式降级成 fallback,主干仍是主键归并」的落地,不是违规**:发 `messageId` 的 harness(omp/opencode)走主键路径,完全不受影响;fallback 仅对无 id 的 harness(Reasonix)生效,属 best-effort(多段无工具间隔的独立消息会被误并,但编码型 agent 单轮罕见)。**原则上的正解仍是 harness 侧补 `messageId`**(Reasonix 内部 `event.Message` 边界信息本就有,只是 ACP adapter 没传);fallback 是「别人不改时我们这边能兜多少」。详见 `docs/worklog/2026-08-02-messageid-fallback-merge.md`。
11. **empty-turn 检测不过度反应(end_turn + 零输出 ≠ 连接坏)**:Prompt 成功返回但 `timeline` 空时,**只 emit error 提示用户、不 teardown/不重连**。`end_turn` 是协议合法返回,连接本身是好的;原来一律 `teardownLive`+`startReconnect` 基于「空 turn = resume 损坏」的过强假设,实测误伤 omp `/review`(client 无 elicitation 时 review 走交互菜单→`select` 返 undefined→命令整体返空→end_turn,详见 `docs/worklog/2026-08-02-empty-turn-not-teardown.md`)。用户看到「本轮无输出」提示后可继续操作(发下条消息、换命令),连接保留。.resume 损坏场景仍能被检测(不静默当成功),只是不再自动拆连接——用户手动重试即可。
12. **交互命令需 elicitation 能力(已支持,见 §3.x)**:部分 harness(omp)的命令(如 `/review`)是 interactive,在 ACP 下经 `available_commands_update` 上报但执行时需要 client 实现 `elicitation/create` 回调(标准协议 v1,SDK 标 UNSTABLE 但可用)。client 不声明 `elicitation.form` 能力时,harness 的 `select/confirm/input` 返回 undefined → 命令静默空(omp 的 bug:hasUI 误判,未降级 headless)。桌面客户端有人在场,支持 elicitation 把这类命令桥接成前端弹窗给用户裁决,正合适(类比 §3.4 权限裁决)。
13. **供应商配额耗尽以 session/prompt 的 JSON-RPC error response 收场,不是断连(已分类处理)**:#46 探针实证——LLM 供应商配额耗尽(如 bigmodel 5h 上限)时 harness 内部 5 次×2 指数退避重试(~33s,对 client 不可见)后,以 `*acp.RequestError{-32603,"Internal error: 已达到…重置。",data:{errorName:"APIError"}}` 返回;**error response 到达 = harness 活着、连接健康**,错误文本自带重置时刻(带内全量信息,无需调接口)。处置(chat.go + acp.DiagnosePromptError):quota→**不 teardown、不重连、不续发队列**,emitErrorDiag 携 `resetAt/rootCause`;transient(429/5xx/超时)→ 同 turn N≤3 指数退避自动重试(对前端不可见),耗尽才 teardown+重连;peer 断/未知→原路径。识别锚定配额文本(稳定),`errorName=="APIError"` 只是弱信号;错误文本按 provider 分中英文(zh=bigmodel,en=Anthropic/OpenAI 系),两边都要锚。详见 `docs/worklog/2026-08-26-quota-exhaustion-probe-46.md` 与 `2026-08-26-prompt-error-classifier-retry-46.md`。
14. **session/fork(UNSTABLE)四铁律(#172 探针实证 + Phase3 裁决)**:①**声明位是唯一门控**——`sessionCapabilities.fork` 未声明不发 RPC(错误码不作依据:jcode 对强发 fork 回过 -32603 而非 -32601);②**同 cwd fork**——cwd 恒传源 session 的 WorkDir,绝不新建 worktree/改 baseRef(换 cwd 在 omp 是确定性错误);fork 行继承源 worktree(自动 guest,删除/合并走既有 guest 规则);③**串行使用**——源 busy(turn 进行中)拒绝 fork,fork 后源与 fork 并发 prompt 不做运行时互斥(omp 实测有崩溃风险,文档化);**空会话(无消息)拒绝 fork**。④**fork 点恒为对话末尾**(协议无位置参数,RFD 预留 messageId 扩展未落地)——UI 分叉入口只放**最后一条 agent 行**,早期消息行不渲染。**fork 行转写 = 血缘查询拼装**(本地 DB 是真相源):`fork_base_seq` 水位列(0024)记录源 fork 时刻最大消息 seq,fork 行 `LoadMessagesPage` 返回 base 前缀(负 seq 偏移)+ 自身消息的单一升序序列,前端分页零改动;**fork RPC 窗口内抑制内容类回放**(opencode 会回放 forked session 的消息且携带新 ACP sessionId,不抑制会污染源会话 UI)。详见 `docs/worklog/2026-08-31-fork-probe-172.md` / `2026-09-01-fork-phase3-review-fixes-172.md`。
