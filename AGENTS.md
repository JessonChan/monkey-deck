...
14. **opencode stdio ACP 不发会话标题(实证);标题须客户端自己生成**(本项目实证 + 协议调研):① ACP 协议**有**标题机制——SDK `SessionUpdate.SessionInfoUpdate`(`SessionSessionInfoUpdate.Title *string`,discriminator `session_info_update`,见 `types_gen.go` + schema.json),`session_info_update` 描述即「Agents send this notification to update session information like title」;`session/new` 的 `NewSessionResponse` **无** title 字段。② **但 opencode 1.17.10 实证不发它**:独立诊断程序抓取一轮完整 turn(+end_turn 后 25s)的全部 `SessionUpdate`,只有 `available_commands_update`/`agent_thought_chunk`/`tool_call`/`tool_call_update`/`agent_message_chunk`,**从不出现 `session_info_update`**。→ 原先 `handleEvent` 监听 `session_info` 想覆盖「首条消息截断标题」的分支**永不触发**,标题永远停在 `maybeAutoTitle` 的 24 字截断。**这是「标题一直用第一句话」的根因。** ③ **修法**(参考 wesight,MIT —— 自生成标题,不依赖 agent):新增 `internal/titlegen`——`FallbackTitle` 纯本地归一化(去 markdown/标签/引号,截断 50 字,**立即**给出);`Generate` 读会话 model 对应的 opencode.json provider 配置(`~/.config/opencode/opencode.json` + cwd,cwd 覆盖全局),调 OpenAI 兼容 `/chat/completions` 异步精修。**chat.go**:`startTurn` 首条消息时先写 fallback 标题(立即推 `chat:session-meta`),再 `go refineTitleAsync` 用 LLM 覆盖(仅当标题仍是 fallback、未被用户改名时)。④ **坑中坑:推理型 model 标题生成会空回**。GLM-5.x / DeepSeek-R1 等推理模型若不关思考,会把 `max_tokens` 全花在 `reasoning_content` 上、`content` 留空(`finish_reason=length`)。**必须**带 `thinking:{type:"disabled"}`;若 provider 不认该字段返 400,自动去掉重试一次(`internal/titlegen` 已做)。**验证**:真实 bigmodel/glm-5.2 配置,标题「README 翻译为中文并添加安装说明」1.6s 返回 ✅;`TestLiveGenerate`(integration tag)+ 单测(纯函数/provider 解析/httptest)✅(2026-06-28)。
